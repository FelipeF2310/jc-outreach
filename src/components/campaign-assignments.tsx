...
export function CampaignAssignments({
    ...
    try {
      ...
      if (request.action !== "workspace") {
        ...
        if (request.action === "event") {
          ...
          setMessage(
            fieldReady
              ? "Assignment saved. Open Volunteer links on its card to share it."
  ...
  return (
    <>
      <section
        id={`assignments-${campaignId}`}
        className="campaign-assignments panel workspace-section"
        aria-label="Event and assignment preparation"
      >
        <p className="eyebrow">ORGANIZE</p>
        <h2>Volunteer assignments</h2>
        <p className="fine">
          Choose doors for each volunteer, then share their private assignment
          link. No volunteer account needed.
        </p>
        {!ready ? (
          <p className="fine">
          ...
          <>
            <div className="workspace-actions assignment-toolbar">
              <button
                className="primary"
                aria-expanded={open}
                disabled={busy}
              ...
              >
                {open ? "Close assignment builder" : "Create assignment"}
              </button>
              <button
                disabled={busy}
            ...
            {workspace && (
              <>
                ...
                <div className="saved-assignment-grid">
                  {workspace.assignments.map((a) => (
                    <div key={a.id} className="saved-assignment">
                      <p>
                        <strong>{a.name}</strong> · {a.householdIds.length}{" "}
                        {a.householdIds.length === 1 ? "door" : "doors"}·{" "}
                        {a.kind === "building"
                          ? "Building run"
                          : "Ordered list"}
                      </p>
                      <p className="fine">
                        {workspace.events.find((e) => e.id === a.eventId)?.name}
                      </p>
                      <details className="assignment-doors">
                        <summary>View assigned doors</summary>
                        {a.householdIds.map((id) => {
                      ...
                      {fieldReady && (
                        <a
                          className="assignment-results-link"
                          href={`#results-${campaignId}`}
                          onClick={() => setResultAssignment(a.id)}
                        >
                          View results
                        </a>
                      )}
            ...
            {workspace?.assignments.length === 0 && (
              <p>
                No assignments yet. Create an event and choose the first
                volunteer’s doors.
              </p>
            )}
            {busy && !loaded && (
            ...
            {open && workspace && (
              ...
              >
                ...
                <details
                  open={eventOpen || workspace.events.length === 0}
                  onToggle={(e) => {
                    if (workspace.events.length)
                      setEventOpen(e.currentTarget.open);
                  }}
                >
                  <summary>
                    {workspace.events.length
                      ? "Create another event"
                      : "First, create a field event"}
                  </summary>
                  <form
                    onSubmit={(event) => {
                  ...
                  >
                    ...
                    <p className="fine">
                      Ends at 5:00 PM New York time on this date, no later than
                      the campaign. Custom end times will be added separately.
                    </p>
                  ...
                  >
                    ...
                    <label className="import-label">
                      ...
                      <select
                        ...
                        onChange={(event) => {
                          setKind(
                            event.target.value as "building" | "scattered",
                          );
                          setPicked([]);
                        }}
                      >
                    ...
                    {doors.map((h) => (
                      <label key={h.id} className="check-row">
                        <input
                          ...
                          disabled={
                            frozen || assigned.has(h.id) || h.suppressed
                          }
                          onChange={(event) =>
                            setPicked((values) =>
