...
async function main() {
  console.info(
    `Synthetic help-queue update started: ${new Date().toISOString()}`,
  );
  if (
  ...
  try {
    ...
    stage = "help_queue_migration_transaction";
    await migrateHelpQueue(postgresDatabase(pool));
    console.info(
      "Synthetic help-queue database update completed. Existing campaigns, links, visits, requests and passwords preserved. No help requests or status updates were created.",
    );
  } finally {
    await pool.end();
  }
...
main().catch((error: unknown) => {
  console.error(
    `Help-queue update failed: stage=${stage}; category=${safeConnectionFailure(error)}.`,
  );
  console.error(
