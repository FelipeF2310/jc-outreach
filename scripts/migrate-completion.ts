...
async function main() {
  console.info(
    `Synthetic completion update started: ${new Date().toISOString()}`,
  );
  if (
  ...
  try {
    ...
    stage = "completion_migration_transaction";
    await migrateCompletion(postgresDatabase(pool));
    console.info(
      "Synthetic completion database update completed. Existing campaigns, links, visits, reports and passwords preserved. No walks were marked finished and no visits were created.",
    );
  } finally {
    await pool.end();
  }
...
main().catch((error: unknown) => {
  console.error(
    `Completion update failed: stage=${stage}; category=${safeConnectionFailure(error)}.`,
  );
  console.error(
