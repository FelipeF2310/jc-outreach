...
Never deploy the local demo authorization as production auth. `JCO_SYNTHETIC_ONLY=1` requires loopback access and refuses Vercel or a configured `DATABASE_URL`. The separate `/api/admin` boundary uses Supabase provider verification and an explicit allowlist in configured hosted modes. Explicit `outreach-live` support and a local restricted-role preload are documented in [LIVE-PRELOAD.md](docs/LIVE-PRELOAD.md); deployment, operator activation and confirmed resident import are distinct milestones. Do not invent actor IDs or import using an owner connection. No raw-upload endpoint exists; `/api/admin/import` accepts built-in synthetic case identifiers only. Read [hosted setup](docs/HOSTED-SETUP.md) before touching account/session/database configuration; owner bootstrap credentials must never be runtime credentials. Never invoke bootstrap on an existing project without explicit owner approval.

The hosted `/api/assignment` and `/api/operations` paths use bearer credentials and reviewed bounded database functions after migration 006, not the administrator session or demo header. The shared operation schema, program fixture and offline client remain common to both backends. Database-side field validation/transaction logic is intentional defense in depth; changes to field semantics require both hosted native-PostgreSQL and local demo tests. Never add an arbitrary-SQL RPC or general runtime resident-table grants to avoid maintaining these boundaries.
