...
Reassignment is now implemented locally. Apply additive update 010 through the hidden-password operator helper, verify restricted-runtime capabilities/read access, then confirm a selected-door handoff and reload with the owner. The new assignment keeps the original event, type and selected-door order; old links and visit history remain. Offline pending uploads remain authorized until the normal window/revocation/expiration boundary. Refreshed lists remove moved doors, including the empty-assignment case. This slice creates a new target assignment rather than merging into an existing one. Completion reporting is the next separate implementation slice. No production CSV or physical-phone gate is closed by these tests.
