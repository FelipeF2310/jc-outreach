...
The owner-confirmed reassignment happy path is complete: Unit 10B moved to B, A kept 2A and all four visits; no credential/visit was created or revoked. An older open page showed 200% until browser reload; the owner confirmed correct 100% and the new notice afterward. The requested guarded update notice is implemented; all 136 automated checks pass and the local app was restarted. It offers explicit reload only when this tab has no open form and shared saved work is received, preserving v1 data; it does not force updates mid-walk. Next separate feature remains completion reporting. Actual multi-build deployment/physical-device update acceptance is still a launch safeguard.
