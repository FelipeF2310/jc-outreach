...
2. Review the exact source commit and pending diff, scan tracked source/history for secrets, then obtain confirmation before pushing the local branch to the public GitHub repository. The configured remote is `FelipeF2310/jc-outreach`; `feat/field-mvp` holds the app locally. Do not import an old main-branch README and assume it contains the working application. No force push or unrelated merge.
