...
- [ ] S10: An app/service-worker/local-schema update preserves unsynchronized records. Compatible-build notification and guarded explicit reload have automated browser coverage using simulated release transport, including pending/draft/rejection blocks, failures, late second-tab saves and retained receipts/offline reopen. Independently built/deployed release transitions, schema migrations and both physical phones remain unverified. Old pre-detector pages require one manual refresh/reload; no automatic reload is claimed.
