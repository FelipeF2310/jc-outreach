...
**Current result: disposable scheduled deletion passed on 2026-09-17.** The owner-run helper confirmed all 20 populated table groups removed, including open help, a successful Cron receipt, rejected old credentials and unchanged practice records/deadlines. Independent READ ONLY verification at 23:18:53 UTC confirms three active campaigns, one deletion, zero overdue/failed campaigns and correct restricted permissions. Do not repeat initialization, installation, schedule setup or this completed rehearsal. Hosted failure/retry and backup/device gates remain open. The installation checkpoints below are historical; the later rehearsal result supersedes their pending instructions.

**Schedule is now enabled; do not repeat setup.** The owner's run beginning 2026-09-17 17:13:51.846 UTC reported success. Independent READ ONLY audits at 17:14:29.381 and 17:16:04.227 UTC observed healthy worker completions at 17:14:00.087206 and 17:16:00.032288 UTC respectively, without manually invoking the worker. Three active campaigns and selected-status reads remain, permissions/provider denial pass, and deleted/overdue/failed counts remain zero. The following setup checkpoints are historical. Automatic execution is verified; operator-only Cron History inspection, actual scheduled expired-fixture deletion and failure/retry remain separate acceptance evidence. Next is a separately approved disposable fixture, never deadline changes to existing practice campaigns.

Latest checkpoint: owner enabled Cron with default installation settings and no jobs. Independent READ ONLY checks at 2026-09-17 17:11:23.216 UTC confirm the extension is installed, all three active synthetic campaigns/status reads work, restrictions remain correct, and no worker run, deletion, overdue campaign or failure is recorded. Continue at step 3 using the ignored `private/enable-retention.command`: read its permanent-deletion warning, confirm **ENABLE-RETENTION**, enter the existing hidden owner password, and share only sanitized output. This enables future cleanup at unchanged campaign deadlines; it is not the install-only helper. Activation is not yet confirmed, and the restricted audit did not inspect cron job history.

The install-only step succeeded in the owner's run beginning 2026-09-17 at 17:06:26.860 UTC. Independent restricted-runtime verification in a READ ONLY transaction passed at 17:08:07.970 UTC: three active campaigns, three selected status reads, correct read-only capability, provider denial, no completed worker run, and zero deleted/overdue/failed campaigns. Cron is not installed and no schedule is enabled. Applied 002–013 are immutable. **Steps 1–2 below are completed historical procedure; continue at step 3.** Do not initialize/reset, rerun the completed install, move existing deadlines or delete practice records to make a test pass.

1. **Install only.** Open the ignored `private/update-retention.command`, type `RETENTION-UPDATE`, then enter the existing database-owner password through its hidden prompt. It invokes `scripts/migrate-retention.ts --password-stdin` with verified TLS, bounded hidden stdin, the existing validated project/CA and sanitized errors. It installs additive 013 in the checksum-checked migration transaction. Existing campaigns, links, visits, reports and passwords are preserved. **It does not delete any campaign or enable the schedule.** Share only its success/failure output, never the password.
2. **Verify read-only.** Engineer verifies `verifyReader`, confirms the runtime can execute `retention_status(uuid)` but cannot execute `run_retention()` or directly read/write retention tables, and reads the status without returning resident records. Refresh the administrator workspace. Expect “Deletion worker is installed; no completed check yet.” Existing assignments and queues must still work. This is not proof that automatic deletion runs.
3. **Separately enable scheduling, after explicit operator confirmation.** Verify the Supabase [Cron module](https://supabase.com/docs/guides/cron/install) is enabled; do not uninstall an existing extension. Review the target project's expired-campaign inventory first. `scripts/migrate-retention.ts --password-stdin --enable-schedule` uses the same secure connection path and installs the fixed `jco-retention-v1` job every minute. It refuses to enable when campaigns are already overdue (those need separate exact-target approval), and refuses to overwrite an existing differing/inactive job. Enabling authorizes permanent deletion of future expired campaign data, including unresolved help requests. The schedule executes database-side, independently of the laptop/app/browser.
4. **Verify actual execution.** Check the job's own history and the application's last-completed check advancing on successive scheduled runs. The administrator should show no overdue campaigns when none are due. No heartbeat after setup is not success. Per-campaign caught failures can yield a successful cron invocation: always inspect the application overdue/failure counts too. The job prunes only its own completed run records older than seven days; it never clears other jobs' history.

The installer only registers the command; it does not itself claim a run completed. A live synthetic deletion rehearsal needs a separately approved, newly created disposable fixture with a near-term deletion deadline, identifiable to the operator. Do not alter or expire the owner's existing campaigns. Verify all its campaign-owned child records are gone, an old fixture link cannot download/upload, no other campaign changed, and the administrator shows the received check. Exercise a disposable-fixture failure/retry in a controlled environment; never add failure triggers to the live practice database without an explicit reviewed plan.

Failure handling: each due campaign is atomic; a child failure leaves the entire campaign pending and stores only its ID/attempt time. Automatic later runs retry. Investigate persistent errors through operator-only diagnostics without logging resident data. Whole-job failure, a stopped schedule, database pause or unavailable health read must not be reported as healthy. The UI distinguishes no first check, stale heartbeat, overdue backlog and deletion failure. There is no administrator HTTP delete/retry button.
...
Recovery follow-up: see [RECOVERY.md](RECOVERY.md) for current provider sources, the remaining inventory and the quarantined restore procedure. The owner reports Free plan, no available scheduled backups and no PITR. The actual local synthetic archive restore/failure/retry test passes, but does not verify Supabase physical recovery, provider backup aging or automatic scheduled retries. No live restore or failure injection has occurred. Physical cloning can immediately start copied jobs, so it must not be treated as a safe inspection sandbox by default. [DEPLOYMENT.md](DEPLOYMENT.md) now covers the next HTTPS synthetic phone-test handoff; nothing is deployed yet.

### Disposable retention rehearsal — hosted success confirmed

Completed with helper checkpoint 7937921: run began 2026-09-17 23:14:53.431 UTC; disposable deadline was 23:16:56.212 UTC. The owner supplied `Scheduled deletion VERIFIED` after seven waiting messages. All descendant, open-help, Cron receipt, old-credential and other-record preservation assertions passed. Independent restricted read-only verification confirms one deletion and three remaining active campaigns. Only synthetic live-table records were permanently removed; backup erasure/recoverability and disconnected-device cleanup were not tested. The following describes the completed procedure, not an instruction to run it again.
...
Subsequent owner walkthrough passed: only Unit 10B moved to Practice Volunteer B, A retained active Unit 2A and all four received visits, and no link or visit was created/revoked. The original tab still ran old code after assignment refresh, showing 200%; full browser reload corrected it to 1 of 1 / 100% and exposed the new reassignment notice without clearing records. The steps below are now historical, not instructions to repeat the move.

The next app build adds guarded compatible-update detection; it needs no database update. After that build is running, an older tab must once use Refresh assignment while online and then browser reload to load the detector. Do not clear site storage. Future detected updates offer Reload to update only after synchronization and closing forms. Validate actual HTTPS release transitions with both phones before launch; simulated browser transport alone does not satisfy that gate.

The owner's run starting 2026-09-15 at 22:29:41 UTC succeeded. Independent read-only verification at 22:30:32 UTC passed the runtime privilege audit, reassignment capability check and imported-workspace reads: three active synthetic campaigns, one imported workspace, one assignment and zero superseded memberships at that historical check. No records/settings were changed by verification. Do not rerun the completed migration; applied 002–010 are immutable. The operator procedure below is historical reference. The subsequent live synthetic handoff/reload walkthrough is owner-confirmed above.
...
### Resident correction review update 009 — applied, live happy path confirmed

The owner's run starting 2026-09-15 at 22:04:05 UTC succeeded. Independent read-only verification at 22:04:51 UTC passed the restricted-runtime audit, both function capability checks and queue reads across three active synthetic campaigns; zero correction reports existed at that point. Do not rerun the completed update. The owner subsequently confirmed report/review/reload/reopening with unchanged visit counts and household membership. The procedure below is retained for reference, not a request to repeat completed work.
...
The restricted-runtime audit and `outreach.correction_queue(uuid)` / `outreach.update_correction_status(uuid,uuid,uuid,integer,text,uuid)` read-only checks have passed. The completed synthetic walkthrough was: create a new visit in a multi-person household, explicitly choose Spoke with resident for the practice scenario, select Person moved for one listed person, then save and synchronize. Review it under Results & follow-up → Resident corrections, mark reviewed and reload; expand Reviewed reports, Keep open, and reload again. The other household members, source records and visit totals must remain unchanged by administrator review. This happy-path walkthrough is owner-confirmed; live failure-path and physical-phone acceptance remain separate.
