...
Latest slice (2026-09-17): retention migration **013 is applied** and **Cron is installed**, independently verified read-only at 17:11:23 UTC. Three active synthetic campaigns remain; the app can read retention status but cannot execute deletion. No worker run or deletion is recorded; no campaigns are overdue. **Schedule activation remains pending.** Next: confirm the separate ENABLE-RETENTION helper, then verify actual scheduled execution. See [retention setup](docs/HOSTED-SETUP.md#retention-update-013--applied-and-read-verified). Real-data, backup and physical-device gates remain open.
