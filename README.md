...
Latest slice (2026-09-17): retention migration **013 is applied and the schedule is enabled**. Independent READ ONLY checks observed worker completions at 17:14 and 17:16 UTC without manual invocation. All three active synthetic campaigns remain, with zero deleted/overdue/failed campaigns and restricted permissions intact. Automatic execution is verified; **actual scheduled deletion of a disposable expired fixture and hosted retry/failure checks remain pending**. See [retention setup](docs/HOSTED-SETUP.md#retention-update-013--applied-and-read-verified). Real-data, backup and physical-device gates remain open.
