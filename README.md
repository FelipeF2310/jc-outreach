...
Household reassignment database update **010** is applied; independent restricted-account capability/workspace-read checks passed on 2026-09-15. **Volunteer assignments → Reassign doors** moves selected active doors into a newly named assignment in the same event, preserving type/order and prior visits. Explicit confirmation is required. No link is automatically issued or revoked. Tell both volunteers about the handoff: disconnected copies cannot update immediately. Refresh removes moved doors from the old list while preserving its saved operations for authorized synchronization. Received visits on reassigned doors remain identifiable in results. The live handoff/reload walkthrough is still pending; completion reporting follows separately.
