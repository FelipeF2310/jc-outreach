...
Household reassignment is implemented and locally tested; additive database update **010** is prepared but not applied. After that update, **Volunteer assignments → Reassign doors** moves selected active doors into a newly named assignment in the same event, preserving type/order and prior visits. Explicit confirmation is required. No link is automatically issued or revoked. Tell both volunteers about the handoff: disconnected copies cannot update immediately. Refresh removes moved doors from the old list while preserving its saved operations for authorized synchronization. Received visits on reassigned doors remain identifiable in results. Hosted operator/live walkthrough evidence is still pending; completion reporting follows separately.

Still required: remaining import failure/retry launch evidence; administrator session/recovery/rate-limit acceptance; production CSV upload/approval handling; hosted reassignment verification and remaining lifecycle checks; remaining live field/follow-up failure-path checks; remaining field details; completion markers; scheduled deletion/failure visibility; client-release migration testing; real-device acceptance; approved program material; production hosting/log/backup review. Local cleanup and server expiry checks alone do not satisfy retention requirements.
