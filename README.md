...
Latest slice (2026-09-17): synthetic campaign retention worker, administrator health/warnings and guarded operator scheduler setup are implemented locally. Migration **013 is not yet applied** and **no hosted deletion schedule is enabled**. Installation preserves existing data and does not run deletion; enabling the every-minute Supabase Cron job is a separate operator action. See [retention setup](docs/HOSTED-SETUP.md#retention-update-013--prepared-not-applied) for the two-step handoff. Real-data, backup and physical-device gates remain open.
