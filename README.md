...
Current CSV checkpoint (2026-09-18): the general CSV engine is implemented locally with bounded memory-only intake/preview, administrator authorization, minimized atomic persistence and actor/content-bound retries. TypeScript, 104 unit/service tests, 18 native PostgreSQL tests and formatting pass, including a generated 1,200-person import through selected assignment and repeat-safe visit sync. **Actual CSV upload is not yet enabled:** migration 014 is unapplied, its runtime capability is ungranted, and no route/UI connects the new adapter. Remaining provider review and deliberate activation are tracked in [CSV intake status](docs/CSV-INTAKE.md). No live data or deployment changed. Earlier checkpoint notes below are historical where superseded.

Logging remediation is applied and independently verified (2026-09-17 EDT): the owner-run role update passed all 13 effective controls, retained restricted permissions and left no old connections. A separate read-only audit at 03:34:59 UTC also passed. Native PostgreSQL canary/rollback/stale-session tests pass. Records, passwords, permissions, other roles and schedules are unchanged; no migration or deployment was needed. See [operator procedure and evidence](docs/UPLOAD-SAFETY.md#remediation--applied-and-verified). Earlier audit findings below are historical. Remaining provider and real-file intake gates are separate.
...
Still required: remaining import failure/retry launch evidence; administrator session/recovery/rate-limit acceptance; production CSV upload/approval handling; remaining hosted lifecycle and field/follow-up failure-path checks; remaining field details; hosted retention failure/retry and backup safeguards; client-release migration testing; real-device acceptance; approved program material; production hosting/log/backup review. Actual hosted scheduled deletion is now verified for a disposable synthetic fixture; it does not close these remaining gates.
