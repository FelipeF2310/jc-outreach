...
Latest slice (2026-09-17): retention migration **013 is applied**, and independent read-only restricted-runtime checks passed at 17:08:07 UTC. Three active synthetic campaigns remain; the app can read retention status but cannot execute deletion. No worker run or deletion is recorded, and the Cron extension is not installed. **No hosted deletion schedule is enabled.** Next: enable the Cron module, then separately confirm schedule registration and verify actual execution. See [retention setup](docs/HOSTED-SETUP.md#retention-update-013--applied-and-read-verified). Real-data, backup and physical-device gates remain open.
